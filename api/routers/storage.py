...
def get_case(trade_id: str,
    ...
    if folder:
        ...
        bpath = Path(folder) / "email_body.txt"
        if bpath.exists():
            try:
                body_excerpt = bpath.read_text(encoding="utf-8")[:1500]
            except Exception:
                pass
    return success({"case": case, "manifest": manifest, "body_excerpt": body_excerpt}, cid)
